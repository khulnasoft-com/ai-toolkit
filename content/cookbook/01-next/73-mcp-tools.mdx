...
Let's create a route handler for `/api/completion` that will generate text based on the input prompt and MCP tools that can be called at any time during a generation. The route will call the `streamText` function from the `ai-toolkit` module, which will then generate text based on the input prompt and stream it to the client.

```ts filename="app/api/completion/route.ts"
import { experimental_createMCPClient, streamText } from 'ai-toolkit';
import { Experimental_StdioMCPTransport } from 'ai-toolkit/mcp-stdio';
