...
- 71f9c51: This adds Vue support for `ai-toolkit` via the `ai/vue` subpath export. Vue composables `useChat` and `useCompletion` are provided.
...
- 78477d3: - Create `/react` sub-package.
  - Create `import { useChat, useCompletion } from 'ai-toolkit/react'` and mark React as an optional peer dependency so we can add more framework support in the future.
